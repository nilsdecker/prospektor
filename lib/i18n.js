...
/** The two sentences the language nudge can say (#114 → #544), in the language
 *  it offers: the one line that IS the link to the twin page, and the name of
 *  the button that closes it. Asked for by `.eleventy.js` on the English pages
 *  only — the nudge never runs anywhere else — for every OTHER built language,
 *  so they are part of the inventory by name. English has no entry: an English
 *  page cannot offer itself. */
const SUGGEST_KEYS = {
...
// that paginates over the languages. Read from the same files the build reads.
function dataStrings() {
  ...
  for (const f of njkFiles(SRC)) {
    ...
    const fm = text.match(/^---\n([\s\S]*?)\n---/);
    if (!fm || !/^pagination:/m.test(fm[1]) || !/^\s+data:\s*languages\s*$/m.test(fm[1])) continue;
...
/** The keys the browser asks for — the subset a page has to ship inline. */
const clientKeys = () => [...new Set(inventory().filter(e => e.kind === 'script').map(e => e.key))];
