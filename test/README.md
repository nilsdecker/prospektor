...
|---|---|---|
| `npm test` | Do the Netlify functions behave? 35 tests over the guards, the fail-open policy and the webhook. | Nothing — no network, no keys, no build |
| `npm run drive` | Does the built site wire up in a real browser? 22 checks over the pay form, the ownership block, the website ask and both fallbacks, with the functions mocked. | Chromium (`CHROME_PATH` to override) |
| `npm run audit` | Is **production** still what the board says it is? 25 claims fetched from the live site. Read-only — it runs a real scan and reads pages, and never posts anything that charges. | Chromium, network (`AUDIT_SITE` to point elsewhere) |
